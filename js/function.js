    ...
    for (let i = 0; i < sizes.length; i++) {
        if (cartItem == null) {
            html += `<li class="${i == 0 ? 'active' : ''}"
                onclick="moveActive(this); updateSize(this, '${sizes[i]}');">${sizes[i]}</li>`;
        } else {
            html += `<li class="${sizes[i] == cartItem.size ? 'active' : ''}"
                onclick="moveActive(this); updateSize(this, '${sizes[i]}');">${sizes[i]}</li>`;
        }
    }
    return html;
...
function createColors(colors, cartItem) {
    ...
    for (let i = 0; i < colors.length; i++) {
        if (cartItem == null) {
            html += `<li style="background-color:${colors[i]};"
                onclick="moveActive(this); updateColor(this, '${colors[i]}');"
                class="${i == 0 ? 'active' : ''}"></li>`;
        } else {
            html += `<li style="background-color:${colors[i]};"
                onclick="moveActive(this); updateColor(this, '${colors[i]}');"
                class="${colors[i] == cartItem.color ? 'active' : ''}"></li>`;
        }
    }
    return html;
}
...
function showImgOfPopup(ele) {
    let img = ele.firstElementChild;
    let src = img.getAttribute('src');
    let target = ele.parentElement.parentElement.previousElementSibling
        .firstElementChild;
...
function insertDataIntoPopup(id) {
    ...
    for (let img of product.images) {
        imagesHtml += `<li class="col" onclick="showImgOfPopup(this)">
            <img src="./../images_Nike/images/products${img}" onerror="this.src='https://placehold.co/150x150/eeeeee/999999?text=x'" alt="" class="img-fluid"></li>`;
    }
    ...
    <div class="row product" data-product-id="${product.id}"
        ...
        <div class="col-md-6">
            <div class="item">
                ...
                <div class="images">
                    <ul class="list-unstyled row">${imagesHtml}</ul>
                </div>
            </div>
        </div>
        <div class="col-md-6">
            <div class="item">
                ...
                <h6>
                    ${product.discount != 0 ? `<span class="before-discount">${product.price} <sup>$</sup></span>` : ''}
                ...
                <h6 class="color my-3">
                    ...
                    <ul class="list-unstyled mb-0">${createColors(product.colors, cartItem)}</ul>
                </h6>
                ${cartItem != null
        ...
        <div class="col-sm-6 col-md-4 product mb-3" data-product-id="${item.id}">
            <div class="item">
                <div class="product-head">
                    <img src="./../images_Nike/images/products${product.images[0]}" onerror="this.src='https://placehold.co/300x300/eeeeee/999999?text=No+Image'" alt="" class="img-fluid">
                </div>
                <div class="product-body">
                    ...
                    <h6 class="size my-3">
                        <strong class="me-3">Size :</strong>
                        <ul class="list-unstyled mb-0"><li class="active">${item.size}</li></ul>
                    </h6>
                    <h6 class="color my-3">
                        <strong class="me-3">Color :</strong>
                        <ul class="list-unstyled mb-0">
                            <li class="active" style="background-color:${item.color};"></li>
                        </ul>
                    </h6>
                    <button class="btn btn-danger d-block w-100"
                        onclick="removeShopProduct(${item.id})">Remove</button>
                </div>
            </div>
        </div>`;
    }
